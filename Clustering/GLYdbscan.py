...
dataGly = dataAll.loc[dataAll["residue name"] == "GLY"]
glyPhi = np.array(dataGly["phi"])
glyPsi = np.array(dataGly["psi"])

# join arrays along a new axis
data = np.stack((glyPhi, glyPsi), axis = 1)

# Compute dbscan
db = DBSCAN(eps=20, min_samples=35).fit(data)
...
plt.title('Estimated number of clusters for GLY: %d' % n_clusters_ )
plt.show() 